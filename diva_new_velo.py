...
class Application(tk.Frame):              
	...
	def read_conf(self):
            ...
            self.user= os.path.basename(self.my_repo)
            self.friends_repo=config.get("git","friends_repo")
            self.friends_repo=self.friends_repo.split(" ")
            self.friends = []
            for friend in self.friends_repo:
                if (friend != ""):
                    self.friends.append(os.path.basename(friend))
	...
	def createWidgets(self):
            ...
            self.image = tk.PhotoImage(file="green.gif")
            self.image2 = tk.PhotoImage(file="red.gif") 
	    self.controlVarDelta=tk.IntVar()
	    self.controlVarState=tk.StringVar()
	    self.controlVarRadio0=tk.IntVar()
	    self.controlVar={}
	    self.radioU={}
	    ...
	    self.radioU0= tk.Radiobutton(self,text=self.user,variable=self.controlVarRadio0,state='normal')
	    self.radioU0.grid(column=0,row=4,sticky='WE')
	    i=1
	    for friend in self.friends:
                self.controlVar[friend] = tk.IntVar()
		self.radioU[friend]= tk.Radiobutton(self,text=friend,variable=self.controlVar[friend],state='disabled')
		self.radioU[friend].grid(column=0,row=4+i,sticky='WE')
		i= i+1
	...
	def calculateGDtot(self):
	    ...
	    Hmax=Hmax|Hi
            ...
            if ((len(Hmax)-len(H1)) < self.sync_limit):
                self.StateLabel.config(foreground='red',image=self.image)
                self.StateLabel.config(foreground='green')
                self.controlVarState.set("OK")
                self.radioU0.config(state='active')
            else:
                self.StateLabel.config(foreground='red',image=self.image2)
                self.StateLabel.config(foreground='red')
                self.controlVarState.set("ATTENTION")
                self.radioU0.config(state='disabled')
